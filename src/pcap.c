...
ssh_pcap_file ssh_pcap_file_new(void)
{
    ...
    pcap = libssh_malloc(sizeof(struct ssh_pcap_file_struct));
    if (pcap == NULL) {
        return NULL;
    }
    ZERO_STRUCTP(pcap);
...
{
    ssh_pcap_context ctx = (struct ssh_pcap_context_struct *)libssh_malloc(sizeof(struct ssh_pcap_context_struct));
    if (ctx == NULL) {
